...
public:
  ...
  void set(state_attribute a) noexcept {
    bits_ |= (1 << static_cast<unsigned char>(a));
  };

  void clear(state_attribute a) noexcept {
    bits_ &= (~(1 << static_cast<unsigned char>(a)));
  };

  bool is_set(state_attribute a) noexcept {
    return bits_ & (1 << static_cast<unsigned char>(a));
  }

  void represent() noexcept {
...
int main() {
  ...
  for (auto at : {state_attribute::absent, state_attribute::clock_prediction,
                  state_attribute::orbit_prediction, state_attribute::special})
    assert(!(f.is_set(at)));
  for (state_attribute at :
       {state_attribute::clock_event, state_attribute::maneuver})
    assert(f.is_set(at));
  f.represent();
  std::cout << "\n";
  return 0;

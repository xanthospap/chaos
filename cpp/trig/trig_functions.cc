...
int main() {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    if (!approxEqual(sina, std::sin(angle))) {
      printf("\n[WARNING] sin diff larger than tolerance: %20.15f for angle "
             "%20.15f (#%5d)",
             rad2deg(sina - std::sin(angle)), rad2deg(angle), sin_probs + 1);
      ++sin_probs;
    }
    if (!approxEqual(cosa, std::cos(angle))) {
      printf("\n[WARNING] cos diff larger than tolerance: %20.15f for angle "
             "%20.15f (#%5d)",
             rad2deg(cosa - std::cos(angle)), rad2deg(angle), cos_probs + 1);
      ++cos_probs;
    }
  }
  printf("\nTested %10d angles; difference larger than tollerance: sin %5.1f%% "
         "cos %5.1f%%",
         samples, cos_probs * 100e0 / samples, sin_probs * 100e0 / samples);
  // test extreme cases

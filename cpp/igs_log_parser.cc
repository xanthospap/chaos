...
public:
      ...
      __last_block_read{-1}
    {
        ...
        {
            throw std::runtime_error{"Cannot open log file: " + std::string(fname)};
        }
    }
    ...
    /// Go to a certain block, identified by its number
    void
    goto_block_nr(int block_nr)
    {
        ...
        if (bstr == block_dict.end()) {
            throw std::runtime_error{"Invalid block number: "+std::to_string(block_nr)};
        }
        ...
        if (!__istream) {
            throw std::runtime_error{"Cannot find block nr: "+std::to_string(block_nr)};
        }
        std::cout<<"\nLast line aread for block nr "+std::to_string(block_nr)+" is: ";
        std::cout<<"\n\""<<__line<<"\"";
        return;
    }
private:
    ...
    int           __last_block_read; ///< The last block read off from file
    pos_type      __stram_pos; ///< The current position in file (to assist reading)
    static constexpr int max_line_chars  {286}; 

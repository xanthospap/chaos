...
private:
  ...
  float f{};
public:
  Class() noexcept {}; // Empty con'tor; we do need this, see tuple_element<N, A::Class>
    ...
    decltype(auto) get() const
  {
    ...
    else if constexpr (N==1) return c;
    else if constexpr (N==2) return f;
  }
...
namespace std {
template<>
  struct tuple_size<A::Class> : integral_constant<size_t, 3> {};
/*
...
template<size_t N>
  ...
  // struct tuple_element<N, A::Class> {using type=decltype(std::declval<A::Class>().get<N>());};
  // else, declval is not needed
  struct tuple_element<N, A::Class> {using type=decltype(A::Class().get<N>());};
}

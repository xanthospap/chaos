    ...
    class running_window
{
    ...
    {
        ++m_cur;
        while ( std::distance(m_start, m_cur) > (m_window/2) ) ++m_start;
        while ( std::distance(m_cur, m_stop) < (m_window/2) && m_stop != m_vec.end() ) ++m_stop;
        return m_cur;
    }
    ...
    get_vec()
    {
        return ( std::vector<T>{m_start, m_stop} );
    }

    std::size_t
    where_am_i()
    {
        return std::distance(m_vec.begin(), m_cur);
    }
...
int main()
{
    ...
    for (auto it = rw.begin(); it != rw.end(); it = rw.next() )
    {
        ...
        for (auto j = w.begin() ; j != w.end(); ++j) { printf(" %3i ",*j); }
        printf("]");
    }

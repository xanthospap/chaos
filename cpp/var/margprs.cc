...
template <int NumArgs = 1> class argument {
public:
  explicit argument(char s, std::string l, std::string v) noexcept
      : _short_opt(s), _long_opt(l), _val(v) {}
  ...
  tvec() {
    ...
    for (std::size_t i = 0; i < tvec.size(); i++)
      tvec.emplace_back(static_cast<T>(std::stod(_val)));
    return tvec;
  }
  ...
  typename std::enable_if<
      std::numeric_limits<T>::is_integer && std::is_signed<T>::value, T>::type
  as_T() const {
...
private:
  ...
  std::vector<std::string> tokenize(char delim = ' ') const {
    ...
    if (start < _val.length()) // ignore trailing delimiter
      v.emplace_back(_val, start,
                     _val.length() - start); // add what's left of the string
    return v;
  }
};

...
A
ret_struct(int i, const std::string& str) noexcept {return A(i, str);}
...
int main()
{
  ...
  // (note that the elements are std::pairs of the key and value type)
  ...
  for (int i=0; i<3; i++) bv1[i] += 1;
  for (int i=0; i<3; i++) assert(bv1[i]==b1.v[i]+1);
  auto& [bi2, bv2] = b1; // bv2 does not decay to pointer; it is a reference to the array
  assert(sizeof(bv2)==sizeof(b1.v));
  for (int i=0; i<3; i++) bv2[i] += 1;
  for (int i=0; i<3; i++) assert(bv2[i]==b1.v[i]);

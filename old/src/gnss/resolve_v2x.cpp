 ...
 *
 */
std::vector<raw_obs> raw_obs::resolve_v2x (const std::string& cstr,std::vector<SatelliteSystem>* ss)
{
  ...
  // mark all possible RINEX v2.xx satellite systems that are of interest (i.e. included
  // in the ss vector. Rinex v2.xx can have GPS, GLO, GAL, SBAS
  bool gps = true;
  bool glo = true;
  bool gal = true;
  bool sbs = true;
  ...
  if ( !(gps+glo+gal+sbs) ) return rtn;

  // match any possible RINEX v2.xx observable string
  switch (ot) {
